...
import com.example.airport.myBatis.dao.PilotMapper;
...
@Model
@SessionScoped
public class PilotsMyBatis implements Serializable {
    ...
    public void loadSelectedPilots(){
        if (airlines.getSelectedAirlineId() != null) {
            this.allSelectedPilots = pilotMapper.findByAirline(airlines.getSelectedAirlineId());
        } else {
    ...
    public void createPilot() {
        if (airlines.getSelectedAirlineId() != null) {
            try {
                pilotToCreate.setAirlineId(airlines.getSelectedAirlineId());
    ...
    public void deletePilot(Pilot pilot) {
        pilotMapper.deleteByPrimaryKey(pilot.getId());
        loadSelectedPilots();
    }
}
